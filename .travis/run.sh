...
flake8 --exclude .tox,.git,__pycache__,pydocs/source/conf.py,docs/source/conf.py,build,dist,tests/fixtures/*,*.pyc,*.egg-info,.cache,.eggs,_package/tests,test_package --ignore=D200,D212 --max-line-length=120 --docstring-convention google --isolated --import-order-style=appnexus --application-import-names=xms.grid --application-package-names=xms --count --statistics .
python build.py

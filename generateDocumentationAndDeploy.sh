...
# conan install -o xmsgrid:pybind=True -s compiler.version=6 -s compiler.libcxx=libstdc++11 -if ./conan -g txt xmsgrid/${TRAVIS_TAG}@aquaveo/stable 
conan install -o xmsgrid:pybind=True -s compiler.version=6 -s compiler.libcxx=libstdc++11 -if ./conan -g txt xmsgrid/1.0.11@aquaveo/stable 
# get the path to the conan package
export PATH_TO_PYTHON_PACKAGE=$(cat ./conan/conanbuildinfo.txt | grep PYTHONPATH.*xmsgrid | sed -r 's/^PYTHONPATH=\["(.*?)"\]$/\1/')
